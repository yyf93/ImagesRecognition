...
用各个模型训练人脸的分类
'''
def train_faces_face_recognition(is_full_train=False):
    """
    使用resnet152去训练人脸
    """
    # 获取图书的配置信息
    ...
    if is_full_train:
        ...
        train_faces_face_recognition_model(face_tool, images, is_full_train)
    else:
        #增量训练
        # 判断是否有新增的图书图像
...
def train_faces_resnet(is_full_train=False):
    """
    使用resnet152去训练人脸
    """
    # 获取图书的配置信息
...
def load_face_recognition_ecodings_names():
    '''
    返回已经训练好的face_recognition的encoding和names
    '''
...
def train_faces_face_recognition_model(face_tool, images, is_full_train):
    '''
    ...
    known_faces_names = []
    # 是否是全量训练， 增量训练需要加上之前的参数
    if not is_full_train:
        if common.is_exit_file(params.params_face_recognition_faces_encoding)\
                and common.is_exit_file(params.params_face_recognition_faces_names):
            # 加载所有人脸图片并获取编码
            known_faces_encodings, known_faces_names = load_face_recognition_ecodings_names()
        else:
            print(f'''不存在模型文件 {params.params_face_recognition_faces_encoding} 或者
            {params.params_face_recognition_faces_names}''')
    # 加载素材， 提取特征用于训练
    for img_path in images:
        ...
        if len(face_locations) > 0:
            if len(face_locations) > 1:
                print(f'训练图像中人脸超过1个， 不予训练， Path: {img_path}')
            else:
                for encoding in face_recognition.face_encodings(face, face_locations):
                    known_faces_encodings.append(encoding)
                    known_faces_names.append(uniq_id)
        else:
            print(f"{img_path} --- No face detected in the image.")
        # 保存训练好的人脸分类器变量
        np.save(params.params_face_recognition_faces_encoding, known_faces_encodings)
        np.save(params.params_face_recognition_faces_names, known_faces_names)
        # 跟新配置文件中该图书已经训练过了
        face_tool.upadte_is_train_to_config(known_faces_names)
        # 评估模型 TODO
...
if __name__ == '__main__':
    train_faces_face_recognition(is_full_train=True)
    # test_faces_face_recognition_model('C:/Users/Administrator/Desktop/ImagesRecognition/images/test_faces/tao_0.jpg')
    # 读取图像数据集和标签
    # images = []

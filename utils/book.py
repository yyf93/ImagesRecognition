...
class BookTool:
        ...
        try:
            ...
            for k, v in book_info_json.items():
                ...
                if k == 'pictures':
                    ...
                    book_info_json[k] = v.replace('\\r', ' ').replace('\\n', ' ').replace('\n', ' ').replace('\r', ' ')
            ...
            self.fill_in_pinyin_engish(book_info_json)
            if book_isbn not in self.isbns:
                print(f"isbn不在配置中 {book_isbn}")
                self.book_name_id[book_name] = self.max_uniq_id
                self.isbns.append(book_isbn)
                self.all_book_infos.append(book_info_json)
                self.current_book_isbn_id_json[book_isbn] = self.max_uniq_id
                self.uniq_ids.append(self.max_uniq_id)
                self.max_uniq_id = str(int(self.max_uniq_id) + 1)
            else:
                print(f"isbn已在配置中 {book_isbn}")
            return True
    ...
    def fill_in_pinyin_engish(self, json_data):
        """
        填充中文的拼音和英文翻译
        """
        name = json_data["bookName"]
        if name:
            #存在中文
            pinyin = json_data["bookPinYin"]
            english_name = json_data["bookEnName"]
            if pinyin:
                if pinyin != '':
                    print("不需要填充拼音")
                else:
                    json_data["bookPinYin"] = common.zh_to_pinyin(name)
            else:
                json_data["bookPinYin"] = common.zh_to_pinyin(name)
            if english_name:
                if english_name != '':
                    ...
                    json_data["bookEnName"] = common.zh_to_en(name)
            else:
                json_data["bookEnName"] = common.zh_to_en(name)
        else:
        ...
        with open(self.book_config_file, 'rb') as f:
            for line in f.readlines():
                ...
                if line != "":
                    ...
                    self.fill_in_pinyin_engish(one_book_json)
    ...
    def get_all_isbns_from_train_dir(self):
        ...
        if len(not_in_config_isbn) > 0:
            ...
            for new_isbn in not_in_config_isbn:
                ...
                if flag:
                    print(f"{new_isbn} 获取新增书籍信息成功")
                else:
                    print(f"{new_isbn} 获取新增书籍信息失败")
    ...
    def get_book_isbn_by_name(self, bookName):
        """
        通过书名获取图书isbn
        """
        for ele in self.all_book_infos:
    ...
    def zh_to_en(self, book_names):
        ...
        for ele in self.all_book_infos:
            ...
            bookEnName = ele['bookEnName']
            if bookName in book_names:
                results.append(bookEnName)
        return results
    ...
    def get_borrow_return_json_file(self, img_file, action, book_name):
        ...
        try:
            ...
            book_id = self.get_book_isbn_by_name(book_name)
            if book_id is None:
                book_id = -1
            # params
            ...
            location_json = f" \"location\": \"{params.book_location}\" "
            one_json = "{" + location_json + ", " + book_id_json + ", " + current_json + ', ' + book_name_json + ", " + action_json + ", " + user_json + ", " + img_json + " }\n"
            # print(one_json)
        ...
        if isbn:
            ...
            if self.is_exit_isbn(isbn):
                ...
                if isbn in self.isbns:
                    info_flag = self.get_book_info_by_isbn(isbn)
                else:
                    info_flag = self.get_book_info_from_api(isbn)
                if info_flag:
                    book_info_json = info_flag
                    # 还是用isbn比较合适
                    id = book_info_json['isbn']
